...
//allows user to save LSystems to a JSON database/file
...
//example set of L Systems being added to the DB
public static class InitialiseDB
{
    public static void Initialise()
    {
        ...
        RuleSet rs10 = new RuleSet("Fractal", "G", "FGH", 90f);
        rs10.AddRule("F", "H-");
        rs10.AddRule("G", "G-F+H&^&F+GFG++G");
        rs10.AddRule("H", "&^HG+-&-F^++G^F^");
        rs10.Validate();

        RuleSet rs11 = new RuleSet("Fractal", "F-F&F&F+F", "FGH", 90f);
        rs11.AddRule("F", "+H+H");
        rs11.AddRule("G", "G+G");
        rs11.AddRule("H", "F-&");
        rs11.Validate();
        ...
        //adding arbitrary 12 systems to the DB for use in the app
